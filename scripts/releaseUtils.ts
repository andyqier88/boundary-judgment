...
export const packages = ['boundary-judgment', 'processaxios', 'bpmnlist'];

export function getPackageInfo(pkgName: string) {
  const pkgDir = path.resolve(__dirname, '../' + pkgName);
  // console.log('__dirname->',__dirname);
  // console.log('pkgName->', path.resolve(__dirname, '../' + pkgName));
...
export async function run(
  ...
  console.log('bin,args,opts->',bin,args,opts);
...
export async function getLatestTag(pkgName: string) {
  ...
  return tags
    .filter((tag: any) => tag.startsWith(prefix))
    .sort()
    .reverse()[0];
}
